...
# ------------------------------------------------------------------
# Cycle through files, first "out" and then "mcrg", "Sout", "eig" and "Wflow"
...
FILE: for my $file (@files) {
  ...
  $infile = "$path/out.$file";
  $check = open IN, "< $infile";
  if (!$check) {
    print STDERR "Problem opening $path/out.$file: $!\n";
    print ERRFILE "Problem opening $path/out.$file: $!\n";
    next FILE;
  }
