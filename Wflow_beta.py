...
# More directly interpolate beta function as function of gc^2 ("gSq"),
...
# ------------------------------------------------------------------
# Carry out interpolations and store results
...
# ------------------------------------------------------------------
# Compute step scaling function and its uncertainty at all gSq
# Then do linear extrapolation to (a / L)^2 --> 0
# Recall s=1.5 is hard-coded
print "# Fitting for %.4g <= u <= %.4g" % (u_min, u_max)

# Uncomment these lines to run two-point continuum extrapolations
#L = np.array([20, 16], dtype = np.int)
#sL = np.array([30, 24], dtype = np.int)
...
for gSq in np.arange(0, u_max, 0.01):   # Preserve uniform spacing
  ...
  for i in range(len(L)):
    ...
    fit = np.poly1d(params[i])          # Define the interpolating function
